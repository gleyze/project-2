...
async function createPolyfillsData(cfg) {
    ...
    if (polyfills.coreJs) {
        addPolyfillConfig({
            ...
            test: utils_1.noModuleSupportTest,
        });
    }
    ...
    if (polyfills.regeneratorRuntime) {
        addPolyfillConfig({
            ...
            test: polyfills.regeneratorRuntime !== 'always' ? utils_1.noModuleSupportTest : undefined,
    ...
    if (polyfills.systemjs ||
        ...
        (!hasSystemJs && utils_1.hasFileOfType(cfg, utils_1.fileTypes.SYSTEMJS))) {
        const name = 'systemjs';
        const alwaysLoad = cfg.modern && cfg.modern.files && cfg.modern.files.some(f => f.type === utils_1.fileTypes.SYSTEMJS);
    ...
    for (const polyfillConfig of polyfillConfigs) {
        ...
        if (polyfillConfig.minify) {
            const minifyResult = await terser_1.default.minify(content, { sourceMap: false });
            // @ts-ignore
            content = minifyResult.code;
        }
        const filePath = `${path_1.default.posix.join(cfg.polyfillsDir || 'polyfills', `${polyfillConfig.name}${polyfills.hash !== false ? `.${utils_1.createContentHash(content)}` : ''}`)}.js`;
        const polyfillFile = {
            name: polyfillConfig.name,
            type: polyfillConfig.fileType || utils_1.fileTypes.SCRIPT,
            path: filePath,
            content,
